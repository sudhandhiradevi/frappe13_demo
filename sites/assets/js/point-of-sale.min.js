(function () {
	...
	erpnext.PointOfSale.ItemSelector = class {
		...
		attach_clear_btn() {
			...
			this.$clear_search_btn.on('click', 'a', function () {
				this$1.set_search_value('');
		...
		bind_events() {
			...
			this.search_field.$input.on('input', function (e) {
				...
				this$1.$clear_search_btn.toggle(
					Boolean(this$1.search_field.$input.val())
				);
			});

			this.search_field.$input.on('focus', function () {
				this$1.$clear_search_btn.toggle(
					Boolean(this$1.search_field.$input.val())
				);
			});
		...
		render_taxes(taxes) {
			if (taxes.length) {
				...
				var taxes_html = taxes.map(function (t) {
					...
					var description = /[0-9]+/.test(t.description) ? t.description : ((t.rate != 0) ? ((t.description) + " @ " + (t.rate) + "%"): t.description);
		...
		attach_refresh_field_event(frm) {
			...
			$(frm.wrapper).on('refresh-fields', function () {
				if (frm.doc.items.length) {
					this$1.$cart_items_wrapper.html('');
				...
				this.warehouse_control.df.onchange = function() {
					...
					if (this.value) {
						me.events.form_updated(me.current_item, 'warehouse', this.value).then(function () {
							...
							var available_qty = me.item_stock_map[me.item_row.item_code][this$1.value][0];
							var is_stock_item = Boolean(me.item_stock_map[me.item_row.item_code][this$1.value][1]);
	...
	erpnext.PointOfSale.Payment = class {
		...
		bind_events() {
			...
			frappe.ui.form.on('POS Invoice', 'contact_mobile', function (frm) {
				...
				if (!this$1.request_for_payment_field) { return; }
			...
			frappe.ui.form.on('POS Invoice', 'coupon_code', function (frm) {
				if (frm.doc.coupon_code && !frm.applying_pos_coupon_code) {
					if (!frm.doc.ignore_pricing_rule) {
						frm.applying_pos_coupon_code = true;
						frappe.run_serially([
							function () { return frm.doc.ignore_pricing_rule=1; },
							function () { return frm.trigger('ignore_pricing_rule'); },
							function () { return frm.doc.ignore_pricing_rule=0; },
							function () { return frm.trigger('apply_pricing_rule'); },
							function () { return frm.save(); },
							function () { return this$1.update_totals_section(frm.doc); },
							function () { return (frm.applying_pos_coupon_code = false); }
						]);
	...
	erpnext.PointOfSale.Controller = class {
		...
		create_opening_voucher() {
			...
			dialog.show();
			var pos_profile_query = function () {
				return {
		...
		async prepare_app_defaults(data) {
			...
			frappe.call({
				...
				callback: function (res) {
					var profile = res.message;
					Object.assign(this$1.settings, profile);
					this$1.settings.customer_groups = profile.customer_groups.map(function (group) { return group.name; });
					this$1.make_app();
				}
			});
		}
		...
		init_item_cart() {
			...
			this.cart = new erpnext.PointOfSale.ItemCart({
				...
				events: {
					...
					checkout: function () { return this$1.save_and_checkout(); },
				...
				events: {
					...
					toggle_other_sections: function (show) {
						if (show) {
							...
							this$1.item_selector.toggle_component(false);
						} else {
							this$1.item_selector.toggle_component(true);
						}
					},
		...
		async make_return_invoice(doc) {
			...
			return frappe.call({
				...
				callback: function (r) {
					...
					this$1.set_pos_profile_data().then(function () {
						frappe.dom.unfreeze();
					});
				}
			});
		...
		async check_stock_availability(item_row, qty_needed, warehouse) {
			var resp = (await this.get_available_stock(item_row.item_code, warehouse)).message;
			var available_qty = resp[0];
			var is_stock_item = resp[1];
		...
		async save_and_checkout() {
			...
			if (this.frm.is_dirty()) {
				...
				await this.frm.save(null, null, null, function () { return save_error = true; });
				// only move to payment section if save is successful
				!save_error && this.payment.checkout();
				// show checkout button on error
				save_error && setTimeout(function () {
					this$1.cart.toggle_checkout_btn(true);
